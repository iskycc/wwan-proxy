...
docker run --detach --privileged \
  ...
  sh -ec '
    apk_last_status=1
    for apk_attempt in $(seq 1 10); do
    ...
    if [[ ${apk_last_status} -ne 0 ]]; then
      echo "apk add openrc ufw nftables failed after 10 attempts" >&2
    ...
    for verify_attempt in $(seq 1 30); do
